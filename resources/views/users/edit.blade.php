        ...
        <form method="POST" action="{{ route('users.update', $user->id) }}" class="bg-surface-dark rounded-2xl border border-border-dark p-8 space-y-6">
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-3 rounded-xl bg-background-dark border border-border-dark text-white focus:border-primary focus:ring-primary">
                    <option value="Administrator" {{ $user->role == 'Administrator' ? 'selected' : '' }}>Administrator - Akses penuh sistem</option>
                    <option value="Manajer" {{ $user->role == 'Manajer' ? 'selected' : '' }}>Manajer - Approve & supervisi</option>
                    <option value="Operator" {{ $user->role == 'Operator' ? 'selected' : '' }}>Operator - Input data</option>
                    <option value="Peninjau" {{ $user->role == 'Peninjau' ? 'selected' : '' }}>Peninjau - Hanya lihat laporan</option>
                </select>
