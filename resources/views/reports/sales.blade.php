<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-white font-display">Laporan Penjualan</h2>
            ...
            <div class="flex items-center gap-3" x-data="{ exportOpen: false }">
                ...
                <div class="relative">
                    ...
                    <div x-show="exportOpen" @click.away="exportOpen = false"
                        ...
                        <a href="{{ route('reports.sales.export-pdf', ['start_date' => $period['start_date'], 'end_date' => $period['end_date'], 'contact_id' => request('contact_id'), 'status' => request('status'), 'unit_id' => request('unit_id')]) }}" 
                        ...
                        <a href="{{ route('reports.sales.export-excel', ['start_date' => $period['start_date'], 'end_date' => $period['end_date'], 'contact_id' => request('contact_id'), 'status' => request('status'), 'unit_id' => request('unit_id')]) }}" 
    ...
    <div class="mb-6 p-4 rounded-xl border border-border-dark bg-surface-dark/30">
        <form action="{{ route('reports.sales') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[150px]">
                <label class="block text-sm text-text-muted mb-2">Pelanggan</label>
                <select name="contact_id" class="w-full bg-surface-highlight border border-border-dark rounded-xl px-4 py-2 text-white focus:border-primary focus:outline-none">
                    ...
                    <option value="{{ $customer->id }}" {{ request('contact_id') == $customer->id ? 'selected' : '' }}>
                        {{ $customer->name }}
                    </option>
                    @endforeach
                </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-surface-dark/50 rounded-xl border border-border-dark p-4">
            <div class="text-text-muted text-sm">Total Penjualan</div>
            <div class="text-2xl font-bold text-primary mt-1">Rp {{ number_format($summary['total_sales'], 0, ',', '.') }}</div>
        </div>
        <div class="bg-surface-dark/50 rounded-xl border border-border-dark p-4">
    ...
    <!-- By Customer Summary -->
    @if($by_customer->count() > 0)
    <div class="mb-6">
        <h3 class="text-lg font-bold text-white mb-3">Penjualan per Pelanggan</h3>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border-dark bg-surface-highlight/30">
                        <th class="text-left text-xs font-medium text-text-muted uppercase tracking-wider px-4 py-3">Pelanggan</th>
